                ...
                try:
                    ...
                    if msg.get("status") == "say":
                        ...
                        if msg.get("api_key"):
                            ...
                            asr.user_info = msg.get("user_info")
                            if not mianshi:
                                mianshi = mianshi_chat('fnbrgwv6_ai_mianshi', msg.get("user_info"))
                        else:
                            asr.client = None

                        import time
                        time.sleep(0.2)
                    elif msg.get("status") == "over" and is_asr:
                        is_asr = False
                        print("收到STOP信号，停止录音")
                        asr.send_finish()
                        if asr.client:
                            mianshi.generate_response(asr.result)
                            ques = mianshi.generate_question()
                            
                            tts_client.create_tts_task(ques)
